...
Unified `PctChange` orders inherit the upstream placeholder-quantity fill accounting because IB computes their total quantity after submission. When the authoritative snapshot is `Ready`, an order for a group saved with `PctChange` must explicitly set both `FaGroup` and `FaMethod = "PctChange"` so that routing is fixed before any fills arrive. Use a saved `Percent` or `Ratio` allocation when exact decimal fill accounting and terminal-status reconciliation are required.
