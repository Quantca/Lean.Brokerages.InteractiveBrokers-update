...
`ib-financial-advisors-unified-groups-enabled` is a deployment-time brokerage setting. LEAN passes it to IBAutomater before IB Gateway starts and the requested state is reapplied on startup and restart, so setting it during algorithm initialization is too late. IBAutomater matches the stable checkbox-label prefix case-insensitively to accommodate punctuation and suffix variations. When the setting is `true`, startup or restart fails closed if the control is unavailable or disabled while unchecked. IBAutomater converges the recognized UI control to the requested state; it does not independently read back the persisted setting or validate downstream IB server behavior.
