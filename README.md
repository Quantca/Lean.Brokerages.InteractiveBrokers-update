...
Unified order-level `PctChange` instructions inherit the upstream placeholder-quantity fill accounting because IB computes their total quantity after submission. Set both `FaGroup` and `FaMethod = "PctChange"` explicitly; `PctChange` is not a saved group allocation method. Saved `ContractsOrShares`, `Ratio`, `Percent`, `NetLiq`, `AvailableEquity`, and `Equal` groups can all receive an explicit order-level `PctChange` instruction. Use a saved `Percent` or `Ratio` allocation with an empty order-level `FaMethod` when exact decimal fill accounting and terminal-status reconciliation are required.
...
The C# and Python `FinancialAdvisorUnifiedGroupsDemoAlgorithm` samples require `ib-financial-advisors-unified-groups-enabled=true` with `ib-financial-advisors-group-filter` empty, because terminal reconciliation retains the original group members as additional account scope. They expect a group saved with a computed method (`Equal`, `NetLiq`, or `AvailableEquity`) or with `Ratio`/`Percent`. A `ContractsOrShares` group instead requires updating its complete saved allocation vector, confirming the readback, and then submitting an aggregate parent whose exact quantity matches that vector's total. `FinancialAdvisorGroupAssignmentAlgorithm` demonstrates the asynchronous membership-mutation/readback state machine only; it does not replace a group's saved allocation vector.
...
Summary data is admitted only through a strict child-account tag whitelist. Every returned member of an overlapping group is structurally and base-currency validated, while only its unresolved candidate members can enter new builders. LEAN falls back to the existing exact per-account `reqAccountUpdatesMulti` path for all unresolved candidates in that group if membership or required rows are incomplete, readiness is not explicitly true, base currencies are mixed or ambiguous, child cash rows disagree, or the aggregate ledger reports any nonzero currency other than the inferred base. A non-base aggregate currency that nets to exactly zero is an accepted detector limitation. Accounts outside selected groups—including unassigned and explicitly retained additional accounts—always use the exact per-account path; rows attributed to nonmembers never enter a child account builder.

The ending topology reread still brackets both summary and fallback collection, so a membership or group-configuration change aborts publication. Cancellation is mandatory because IB permits only two account-summary subscriptions. If cancellation is ambiguous, LEAN keeps the already validated current-group result but disables further summary batching until the next physical reconnect and completes unresolved accounts through the exact path. These batching details do not change `GetAccountSnapshot`, `RequestAccountSnapshotRefresh`, or the contents and freshness semantics of a published snapshot; they reduce the normal grouped-account request count while preserving the exact path as the fail-closed authority.
