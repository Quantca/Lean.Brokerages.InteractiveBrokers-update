...
The C# and Python `FinancialAdvisorDemoAlgorithm` samples require `ib-financial-advisors-unified-groups-enabled=true` with `ib-financial-advisors-group-filter` empty, because terminal reconciliation retains the original group members as additional account scope. They expect a group saved with a computed method (`Equal`, `NetLiq`, or `AvailableEquity`) or with `Ratio`/`Percent`. A `ContractsOrShares` group instead requires updating its complete saved allocation vector, confirming the readback, and then submitting an aggregate parent whose exact quantity matches that vector's total. Use `FinancialAdvisorGroupAssignmentAlgorithm` as the mutation/readback workflow reference for this case.
