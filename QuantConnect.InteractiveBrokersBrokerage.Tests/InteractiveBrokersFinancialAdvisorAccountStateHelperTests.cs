...
namespace QuantConnect.Tests.Brokerages.InteractiveBrokers
{
    ...
    public class InteractiveBrokersFinancialAdvisorAccountStateHelperTests
    {
        ...
        [TestCase(BrokerageAccountRelationship.Primary, "PaperMaster")]
        [TestCase(BrokerageAccountRelationship.Aggregate, "PaperMasterA")]
        public void UnsupportedGroupRelationshipProvidesActionablePrerequisite(
            ...
            var exception = Assert.Throws<
                InteractiveBrokersFinancialAdvisorAccountState
                    .UnsupportedFinancialAdvisorConfigurationException>(() =>
                        InteractiveBrokersFinancialAdvisorAccountState
                            .ValidateSupportedGroupAccountRelationships(groups, directory));

            StringAssert.Contains(group.Name, exception.Message);
            StringAssert.Contains(accountId, exception.Message);
            StringAssert.Contains(
                relationship.ToString().ToLowerInvariant(),
                exception.Message);
            StringAssert.Contains("managed subaccounts only", exception.Message);
            StringAssert.Contains("Remove this account from the group in TWS", exception.Message);
            StringAssert.Contains("refresh the brokerage account snapshot", exception.Message);
        }

        [TestCase(BrokerageAccountRelationship.Managed)]
        [TestCase(BrokerageAccountRelationship.Unknown)]
        public void SupportedGroupRelationshipIsAccepted(
            BrokerageAccountRelationship relationship)
        {
            var group = new BrokerageAccountGroup(
                "Group",
